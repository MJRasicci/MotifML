...
def _coerce_onset_group(
    ...
    offset = _coerce_score_time(
        raw_beat.get("offset"),
        path=f"{context.beat_path}.offset",
        diagnostics=diagnostics,
        require_positive=False,
    )
    duration = _coerce_score_time(
        raw_beat.get("duration"),
        path=f"{context.beat_path}.duration",
        diagnostics=diagnostics,
        require_positive=True,
    )
    notes = _require_list_field(
        raw_beat,
        field_name="notes",
        path=f"{context.beat_path}.notes",
        diagnostics=diagnostics,
    )
    ...
    if offset >= context.bar_duration:
        diagnostics.append(
            _error(
                path=f"{context.beat_path}.offset",
                code="invalid_canonical_field",
                message=(
                    f"beat offset exceeds the duration of bar {context.bar_index}."
                ),
            )
        )
        return None

    if offset + duration > context.bar_duration:
        diagnostics.append(
            _error(
                path=f"{context.beat_path}.duration",
                code="invalid_canonical_field",
                message=(
                    "beat duration exceeds the remaining space in "
                    f"bar {context.bar_index}."
                ),
            )
        )
        return None
